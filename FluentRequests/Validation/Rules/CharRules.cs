...
namespace FluentRequests.Lib.Validation.Rules
{
    public static class CharRules
    {
        ...
        public static Rule<char> IsDigit(ValidationState state = null)
            => new Rule<char>(c => char.IsDigit(c), state: state);

        [Convert]
        [Test(validTestCases: new string[] { "'^'", "'|'", "'='", "'<'", "'>'", },
              invalidTestCases: new string[] { "'1'", "'2'", "true", "1", "1L", })]
        public static Rule<char> IsSymbol(ValidationState state = null)
            => new Rule<char>(c => char.IsSymbol(c), state: state);

        [Convert]
        [Test(validTestCases: new string[] { "'A'", "'b'" },
            invalidTestCases: new string[] { "'^'", "'|'", "'1'", "'2'", "false", "true" })]
        public static Rule<char> IsLetter(ValidationState state = null)
            => new Rule<char>(c => char.IsLetter(c), state: state);
        ...
        public static Rule<char> In(params char[] acceptableLetters)
            => new Rule<char>(c => acceptableLetters.Contains(c), state:RulesStates.Warn);
    }
}
